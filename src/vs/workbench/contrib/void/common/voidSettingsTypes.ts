...
export const subTextMdOfProviderName = (providerName: ProviderName): string => {
	...
	if (providerName === 'gemini') return 'Get your [API Key here](https://aistudio.google.com/apikey). Read about [rate limits here](https://ai.google.dev/gemini-api/docs/rate-limits#current-rate-limits). You can optionally set a custom API URL for proxy or alternative endpoints.'
